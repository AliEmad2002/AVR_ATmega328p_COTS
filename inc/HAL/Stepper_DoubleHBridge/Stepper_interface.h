...
// NOTE! :
// THITS DRIVER USES TIMER/COUNTER 1 IN ITS OPERATION FOR ALL STEPPER OBJECTS
// IN NORMAL MODE, BOTH CHANNELS DISABLED, PLEASE AVOID USING/INITIALIZING TIMER 1 WHEN USING THIS DIRVIER.
...
{
	u8 pins[4] ;
	u8 pinEN ;	// to stop current flow when motor is temporarily not in use
...
//
// 2:
// What is the difference between stepper turned on but not stepping, and stepper turned off?
// When stepper is turned on but not stepping, it maintains its position against any outer mechanical effect due to lenz's law.
// While when it's turned off, it can't maintain its position against any outer mechanical effect.
//
// 3:
//
//
